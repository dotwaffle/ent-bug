...
func (tc *ThingCreate) SetAge(i int) *ThingCreate {
...
func (tc *ThingCreate) AddProbedByIDs(ids ...int) *ThingCreate {
	tc.mutation.AddProbedByIDs(ids...)
	return tc
}

// AddProbedBy adds the "probed_by" edges to the ThingHTTP entity.
func (tc *ThingCreate) AddProbedBy(t ...*ThingHTTP) *ThingCreate {
	ids := make([]int, len(t))
	for i := range t {
		ids[i] = t[i].ID
	}
	return tc.AddProbedByIDs(ids...)
}
...
func (tc *ThingCreate) check() error {
	if _, ok := tc.mutation.Age(); !ok {
		return &ValidationError{Name: "age", err: errors.New(`ent: missing required field "Thing.age"`)}
	}
...
func (tc *ThingCreate) createSpec() (*Thing, *sqlgraph.CreateSpec) {
	...
	if nodes := tc.mutation.ProbedByIDs(); len(nodes) > 0 {
